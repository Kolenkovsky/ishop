<td class="nav">
					...
					<ul class="navigation">
						...
						<? if($pages) : ?><!Так же как и в news проверяем пришла ли перпеменная pages иформируем ссылки>
                            <? foreach($pages as $item) : ?>
                               <li>&mdash;&nbsp; <a href="<?=SITE_URL;?>page/id/<?=$item['pages_id']?>"><?=$item['title']?></a></li>
                            <? endforeach; ?>
                        <? endif; ?>
						...
						<? if($brands) : ?>
                            <div id="list_brands">
                                <? foreach($brands as $key=>$item_b) : ?>
                                    ...
                                    <? if($item_b['next_level']) : ?><!Проверяем а есть ли дочернии категории у этой категории>
                                        <p><a href="#"><?=$item_b[0];?></a></p><!Выводим родит.кат.ссылки тут нет она просто акардионом открывает список>
                                        
                                        <!Теперь уже пройдемся циклом по дочерней категории>
                                        <div>
                                            <a href="<?=SITE_URL;?>catalog/parent/<?=$key;?>">Все типы</a><br />
                                        <? foreach($item_b['next_level'] as $k=>$v) : ?>
